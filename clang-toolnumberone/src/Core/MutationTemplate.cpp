...
public:
  ...
  void applyMutations(const MatchFinder::MatchResult &Result) {
    ...
    for (MutatorType i = 0; i < this->mutator->getTypes(); ++i) {
      ...
      if (localRw.getRewriteBufferFor(localRw.getSourceMgr().getMainFileID()) !=
        ...
        ToolnumberoneLogger::verboseAndIncr("[" + std::to_string(mutantId) +
                                      "][ RUN  ] Checking mutant");

        if (this->checkMutant(localRw)) {
          ToolnumberoneLogger::verbosePreDecr("[" + std::to_string(mutantId) +
  ...
  bool saveMutant(mutant::IdType id, Rewriter &rewriter) {
    ...
    std::string mutantPath = this->mutationTemplate.getTargetOutputDirectory() +
                             std::to_string(id) + toolnumberone::fs::pathSep;
    std::string filePath = mutantPath + filename;
    ToolnumberoneLogger::verbose("[" + std::to_string(id) + "] Saving mutant in " +
                           clang::tooling::getAbsolutePath(filePath));

    // Create folder for this mutant
    toolnumberone::fs::createDirectories(mutantPath);
    ...
    if (!file.has_error()) {
      ...
      ToolnumberoneLogger::error("An error occurred during the file opening: " +
                           fileError.message());
      return false;
    }
  ...
  bool checkMutant(Rewriter &rw) {
    ...
    std::string tempDir = this->mutationTemplate.getTargetOutputDirectory() +
                          this->tempDirName + toolnumberone::fs::pathSep;
    std::string tempFilePath =
        tempDir + this->mutationTemplate.getTargetFilename().data();

    // Create the directory
    toolnumberone::fs::createDirectories(tempDir);
    ...
    if (!tempFile.has_error()) {
      ...
      ToolnumberoneLogger::verbose("Building CompilationDatabase");
      ...
      ::toolnumberone::cd_utils::changeCompileCommandTarget(
...
#ifdef _CHIMERA_DEBUG_
      toolnumberone::cd_utils::dump(std::cout, commands); // Debug
#endif
      ToolnumberoneLogger::verbose("Running syntax check");
      ...
      return toolnumberone::checkSyntaxAction(
  ...
  void createReportEntry(mutant::IdType id, const std::string &functionName,
    ...
    ToolnumberoneLogger::verbose("[" + std::to_string(id) +
  ...
  virtual void run(const MatchFinder::MatchResult &Result) {
    ToolnumberoneLogger::verboseAndIncr("Coarse grain matching from " +
    ...
    if (this->mutator->match(Result)) {
      ...
      ToolnumberoneLogger::verboseAndIncr("Fine grain matching [ PASS ]");

      // With the introduction of the HOM mutators, this phase has to be
      // specialized
      this->applyMutations(Result);

      ToolnumberoneLogger::decrActualVLevel();
    } else {
      ToolnumberoneLogger::verbose("Fine grain matching [ FAIL ]");
    }
    ToolnumberoneLogger::decrActualVLevel();
  }
  /**
  ...
  virtual void onEndOfTranslationUnit() {
    //    ToolnumberoneLogger::verbose(" [ RUN  ] Cleaning up");
    ...
    if (this->mutator->isHom() && this->localMutantId != 0 &&
      ...
      this->mutator->onCreatedMutant(
          ...
          ::std::to_string(this->localMutantId) + ::toolnumberone::fs::pathSep);
    }
    ...
    //    ToolnumberoneLogger::verbose(" [ DONE ] Cleaning up");
  }
...
// Private methods
void toolnumberone::MutationTemplate::initMutantIds_() {
  // Reset slot manager
  ...
  for (auto op : this->operators) {
    if (op.second->isHom()) {
      ...
      if (!idManager.setSlot(op.second->getIdentifier(), reservedId) ||
        ...
        ToolnumberoneLogger::fatal("Couldn't reserve a mutantId for an operator. "
...
void toolnumberone::MutationTemplate::addMatchers_(
  ...
  if (this->operators.at(operatorId)->isHom()) {
    ...
    if (!idManager.getReservedSlot(operatorId, reservedId)) {
      ToolnumberoneLogger::fatal("An id wasn't reserved for this operator.");
    }
  }
...
void toolnumberone::MutationTemplate::addMatchers_(
  ...
  if (operatorId != operatorIds.end()) {
    ...
    ToolnumberoneLogger::verbose("Found CHIMERA_ALL_OPERATORS");
    {
...
int toolnumberone::MutationTemplate::run(clang::ast_matchers::MatchFinder &finder) {
  int retval = 1; // Default error
  if (isGenerateMutants() || isGenerateMutantsReport()) {
    ToolnumberoneLogger::verboseAndIncr("[ RUN  ] Internal tool");
    
    // Create output folder
    ToolnumberoneLogger::verbose(
        "Creating output folder " +
        clang::tooling::getAbsolutePath(this->getTargetOutputDirectory()));
    if (!toolnumberone::fs::createDirectories(this->getTargetOutputDirectory())) {
      ToolnumberoneLogger::fatal("Couldn't create output folder");
      return 1;
    }
    ...
    if (this->openReportStream("report.csv")) {
      ...
      retval = (ClangTool(::toolnumberone::cd_utils::FlexibleCompilationDatabase(
                              this->compileCommand),
                          this->targetPath))
      ...
      ToolnumberoneLogger::fatal("Couldn't open the report file");
    }
    ToolnumberoneLogger::decrActualVLevel();
    ToolnumberoneLogger::verbose("[ DONE ] Internal tool");
  } else {
    ToolnumberoneLogger::verbose("[ SKIP ] Running internal tool is useless, nor "
...
toolnumberone::MutationTemplate::MutationTemplate(
    ...
    : mutantCounter(mutantCounterInitial), compileCommand(compileCommand),
      ...
      tool(toolnumberone::cd_utils::FlexibleCompilationDatabase(this->compileCommand),
           targetPath),
...
// target
#ifdef _CHIMERA_DEBUG_
  toolnumberone::cd_utils::dump(std::cout, this->compileCommand);
#endif
  toolnumberone::log::ToolnumberoneLogger::verbosePreDecr(
      "[ DONE ] Building MutationTemplate");
}

int toolnumberone::MutationTemplate::analyze() {
  this->initMutantIds_();
  // Create a new finder
  MatchFinder finder;
  ToolnumberoneLogger::verbose(
...
int toolnumberone::MutationTemplate::analyze(const conf::FunOpConfMap &map) {
  this->initMutantIds_();
  // Create a new finder
  MatchFinder finder;
  ...
  if (row != map.end()) {
    ...
    ToolnumberoneLogger::verbose("Found CHIMERA_ALL_FUNCTIONS");
...
bool toolnumberone::MutationTemplate::openReportStream(const char *reportName) {
  this->reportStream.open(this->getTargetOutputDirectory() + reportName,
                          std::ofstream::out);
  return this->reportStream.is_open();
}

std::ostream &toolnumberone::MutationTemplate::getReportStream() {
  return this->reportStream;
}

void toolnumberone::MutationTemplate::closeReportStream() {
