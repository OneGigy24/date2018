...
bool toolnumberone::perforation::MutatorLoopPerforation1::getMatchedNode(
    const toolnumberone::mutator::NodeType &node,
...
toolnumberone::perforation::MutatorLoopPerforation1::getStatementMatcher()
{
 return stmt(
...
bool toolnumberone::perforation::MutatorLoopPerforation1::match(
    const ::toolnumberone::mutator::NodeType &node)
{
...
::clang::Rewriter &toolnumberone::perforation::MutatorLoopPerforation1::mutate(
    const ::toolnumberone::mutator::NodeType &node,
    ::toolnumberone::mutator::MutatorType type, ::clang::Rewriter &rw) 
{ 
  bool inc = true; 
  ...
  if(this->binc){
    switch (mapOpCode(this->binc->getOpcode())){
      ...
      break;
      default :
        ToolnumberoneLogger::verbose("OpCode sconosciuto: " + std::to_string(this->binc->getOpcode())); 
      break;
    } 
  }else{
    if(this->inc){
...
void ::toolnumberone::perforation::MutatorLoopPerforation1::clean()
{
  this->cond = this->binc = this->bas = nullptr;
  this->inc = nullptr;
}

void ::toolnumberone::perforation::MutatorLoopPerforation1::onCreatedMutant(
