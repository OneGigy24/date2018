...
///\{
// Private methods
void toolnumbertwo::engine::ExecutionEngineHelper::createExecutionEngine(
    ::std::unique_ptr<::llvm::Module> M)
{
  ::toolnumbertwo::log::ToolnumbertwoLogger::verbose(
  ...
  this->ee.reset(::llvm::EngineBuilder(::std::move(M))
                     ...
                     .create());
  if (!this->ee) {
    ::toolnumbertwo::log::ToolnumbertwoLogger::error
...
int toolnumbertwo::engine::ExecutionEngineHelper::compileDatabaseAndLoad(
         ...
         const ::std::string &Path)
{
  ...
  if (std::error_code EC = FileOrErr.getError()){
    ::toolnumbertwo::log::ToolnumbertwoLogger::error("Couldn't create memory buffer\n");
...
void toolnumbertwo::engine::ExecutionEngineHelper::loadArchive(const ::std::string &path)
{
  ::toolnumbertwo::log::ToolnumbertwoLogger::verbose("Trying to load archive: " + path +  "...");
  if (::llvm::sys::fs::exists(path)) {
    this->ee->addArchive(createArchive(path));
    ::toolnumbertwo::log::ToolnumbertwoLogger::verbose("Done\n");
  } else {
    ::toolnumbertwo::log::ToolnumbertwoLogger::verbose("Fail, it doesn't exist\n");
  }
}

void ::toolnumbertwo::engine::ExecutionEngineHelper::loadArchives (
...
void toolnumbertwo::engine::ExecutionEngineHelper::loadObj(const ::std::string &objPath)
{
  log::ToolnumbertwoLogger::verbose("Trying to load: " + objPath + "...");
  if (::llvm::sys::fs::exists(objPath)) {
    ...
    if(objPath.substr(objPath.find_last_of(".") + 1) == "so") {
      //Shared object (.so)
      log::ToolnumbertwoLogger::verbose("Loading Shared Object...");
      ...
      *error = "NO ERROR!";
      ...
      if(so.LoadLibraryPermanently(s, error)) { 
        ...
        log::ToolnumbertwoLogger::verbose("LoadLibraryPermanently ERROR: " + *error);
      } else {
        log::ToolnumbertwoLogger::verbose("Shared object loaded!");
      }
      delete s;
    } else {
      //Normal object (.o)
      log::ToolnumbertwoLogger::verbose("Loading Object..."); 
      if(auto objOr = ::llvm::object::ObjectFile::createObjectFile(objPath)) {
        auto &obj = *objOr;
        this->ee->addObjectFile(objOr.get().takeBinary().first);
        log::ToolnumbertwoLogger::verbose("object loaded!");
      }else {
        log::ToolnumbertwoLogger::verbose("ERROR: Failed to open object!");
      }
    }
  }else {
    log::ToolnumbertwoLogger::verbose("Failed, object doesn't exist!");
  }
}

void toolnumbertwo::engine::ExecutionEngineHelper::loadObjs(
...
bool toolnumbertwo::engine::ExecutionEngineHelper::loadModule(
    ::std::unique_ptr<::llvm::Module> M)
{
  ::llvm::Module &m = *M;
  if (!this->ee) {
...
bool toolnumbertwo::engine::ExecutionEngineHelper::setCompilationDatabase(
    const ::std::string &path) {
  ::std::string error;
...
bool toolnumbertwo::engine::ExecutionEngineHelper::hasCompileCommand(
...
bool toolnumbertwo::engine::ExecutionEngineHelper::checkFunction(::llvm::StringRef funcName){
    return 0 != this->ee->getFunctionAddress(funcName);
}

// IMPORTANTE
int toolnumbertwo::engine::ExecutionEngineHelper::runFunction(
...
int toolnumbertwo::engine::ExecutionEngineHelper::compileAndRunFunction(
...
uint64_t toolnumbertwo::engine::ExecutionEngineHelper::getAddress(::llvm::StringRef Name) {
  // - Execution engine is set
