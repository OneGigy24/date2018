...
::toolnumbertwo::core::AprxGrade loopContext::LoopAprxContext::getMaxApplicableGrade() const
{
...
bool loopContext::LoopAprxContext::readReport(::std::string reportPath)
{
  ...
  if (reportPath == "") {
    log::ToolnumbertwoLogger::error("Empty report path");
    return false;
  }
  ...
  log::ToolnumbertwoLogger::verbose("Reading a LOOP report:\n");
  while (loopReport.read_row(OpId, line, inc, lenght)) {
    log::ToolnumbertwoLogger::verbose(
      "OperationId: " + OpId); 
