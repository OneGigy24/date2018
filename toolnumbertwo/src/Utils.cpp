...
bool toolnumerone::conf::readFunctionsOperatorsConfFile(
...
// Filesystem
const char toolnumerone::fs::pathSep = getPathSeparator();

char toolnumerone::fs::getPathSeparator() {
  return *(sys::path::get_separator().data());
}

std::string toolnumerone::fs::getParentPath(const std::string& path) {
  // Use this trick because llvm::sys::path::parent_path doesn't work
  std::size_t found = path.find_last_of(pathSep);
  return path.substr(0, found);
}

bool toolnumerone::fs::createDirectories(const llvm::Twine& path,
                                    bool ignoreExisting) {
  // Create directories
